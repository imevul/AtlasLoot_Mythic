...
local function IsArmorTypeValid(itemType, itemSubType, itemLevel)
...
local function CheckItemValid(itemId)
    if not itemId or itemId <= 0 or ItemAttuneSkip == nil or not ItemAttuneHas then return 0 end
	...
	if (dataID == "SearchResult") or (dataID == "WishList") or (AtlasLoot_IsLootTableAvailable(dataID)) then
		...
		for i = 1, 30, 1 do
			...
			if(dataSource[dataID][i] ~= nil and dataSource[dataID][i][4] ~= "") then
				...
				if dataSource[dataID][i][2] ~= "" and dataSource[dataID][i][2] ~= 0 and ItemAttuneHas and (ItemAttuneHas[dataSource[dataID][i][2]] or 0) < 100 and CheckItemValid(dataSource[dataID][i][2]) > 0 then
					text = "!! "..text;
				end
...
function AtlasLoot_GetLODModule(dataSource)
	...
	elseif (dataSource=="AtlasLootMythic") then
		return "AtlasLoot_Mythic";
	end
end
...
function AtlasLoot_LoadAllModules()
	local orig, bc, wotlk, craft, world, myth;
	...
	myth, _ = LoadAddOn("AtlasLoot_Mythic");
    local flag=0;
	if not orig then
